...
The \code{restoreopt} function will allow to reset the options to the state when \code{restauraropt_mk} was called. To do that, execute:
...
```{r beta1, fig.width=13, fig.height=12, warning=FALSE, message=FALSE}
library(cowplot)
...
```{r beta2, fig.width=12, fig.height=4, warning=FALSE}
library(cowplot)
...
Perform PERMANOVA with the \code{vegan::adonis2} function:
...
Perform PERMANOVA with the \code{vegan::adonis2} function, using covariates:
...
First, use Sex and Age as covariates. We will use the \code{phyloseq} object with the raw counts, since \code{DESeq2} normalizes data internally.

Several tables and plots will be saved to the \code{opt$out} directory. 

```{r deseq1, fig.width=12, fig.height=4, warning=FALSE, error=FALSE, fig.show='hide'}
data("phobj_filtonly")
...
```{r deseq4, fig.width=12, fig.height=4, warning=FALSE, error=FALSE}
...
```{r deseq5, fig.width=12, fig.height=4, warning=FALSE, error=FALSE}
...
```{r deseq6, fig.width=12, fig.height=5, warning=FALSE, error=FALSE}
...
```{r deseq7, fig.width=12, fig.height=4, warning=FALSE, error=FALSE, fig.show='hide'}
